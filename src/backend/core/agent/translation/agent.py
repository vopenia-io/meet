...
class TranscriptionAgent():
    def __init__(self, room, meta: TranslationMeta):
        self._room = room
        self._meta = meta
        self._stt = gladia.STT(
            languages=None,
            translation_enabled=bool(meta.lang),
            interim_results=False,
            translation_target_languages=list(meta.lang) if meta.lang else None,
            energy_filter=False
        )
    ...
    async def _process_stt_stream(self, stream: SpeechStream, participant: rtc.Participant, track: rtc.RemoteTrack):
        try:
...
async def entrypoint(ctx: agents.JobContext):
    ...
    agent = TranscriptionAgent(room, meta)
